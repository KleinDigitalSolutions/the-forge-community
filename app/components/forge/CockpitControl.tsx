  ...
  return (
    <div ref={containerRef} className="relative flex items-center justify-center w-[min(720px,90vw)] h-[min(720px,90vw)]">
      ...
      {!minimal && (
        ...
        >
          <div className={`transition-all duration-1000 absolute w-[300px] h-[300px] bg-white/5 rounded-full ${isOpen ? 'scale-150 opacity-40' : 'scale-100 opacity-20'}`} />
          
          {/* Rotating Tech Rings (Subtle) */}
          <motion.div 
            animate={{ rotate: 360 }}
            transition={{ duration: 100, repeat: Infinity, ease: "linear" }}
            className="absolute w-[min(640px,80vw)] h-[min(640px,80vw)] rounded-full border border-white/10 opacity-30"
          />
        </div>
      )}
      ...
      <div className="relative z-50">
          ...
          {!isOpen && (
            ...
            >
              <motion.div
                ...
                className="absolute inset-[-20px] rounded-full border-2 border-white/15"
              />
              <motion.div
                animate={{ 
                  scale: [1, 1.2, 1],
                ...
                className="absolute inset-[-10px] rounded-full bg-white/5 border border-white/10"
              />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        >
          ...
          {!minimal && (
            <div className="absolute inset-[-8px] rounded-full border border-white/10 border-t-white/30 animate-spin-slow pointer-events-none" />
          )}
        </motion.button>
  ...
  return (
    <div className="flex flex-col items-center gap-2 group">
      ...
      <div className="relative w-12 h-12 flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
        <item.icon className="w-7 h-7 text-white/70 group-hover:text-white" />
        {badge > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-1 rounded-full bg-white/80 text-[8px] font-black text-black flex items-center justify-center">
            {badgeText}
          </span>
        )}
      </div>
      <div className="text-center antialiased">
        <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/80 group-hover:text-white transition-colors">
          {item.label}
        </div>
