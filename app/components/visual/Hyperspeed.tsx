...
type HyperspeedOptions = {
  onSpeedUp?: (event?: Event) => void;
  onSlowDown?: (event?: Event) => void;
  distortion: string;
  length: number;
  roadWidth: number;
  islandWidth: number;
...
export default function Hyperspeed({
  ...
  useEffect(() => {
    ...
    class App {
      ...
      onTouchStartBound: (event: TouchEvent) => void;
      onTouchEndBound: (event: TouchEvent) => void;
      onContextMenuBound: (event: MouseEvent) => void;

      constructor(containerEl: HTMLDivElement, options: any = {}) {
        ...
        this.renderer.setSize(containerEl.offsetWidth || 1, containerEl.offsetHeight || 1, false);
        ...
        this.camera = new THREE.PerspectiveCamera(
          options.fov,
          (containerEl.offsetWidth || 1) / (containerEl.offsetHeight || 1),
          0.1,
          10000
        );
      ...
      init() {
        ...
        this.container.addEventListener('touchstart', this.onTouchStartBound, { passive: true });
        this.container.addEventListener('touchend', this.onTouchEndBound, { passive: true });
        this.container.addEventListener('touchcancel', this.onTouchEndBound, { passive: true });
      ...
      onMouseDown(event: MouseEvent) {
        if (this.options.onSpeedUp) this.options.onSpeedUp(event);
        this.fovTarget = this.options.fovSpeedUp;
        this.speedUpTarget = this.options.speedUp;
      }

      onMouseUp(event: MouseEvent) {
        if (this.options.onSlowDown) this.options.onSlowDown(event);
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          const leftX = laneX + carShiftX - carWidth / 2;
          const rightX = laneX + carShiftX + carWidth / 2;

          const offsetY = random(options.carFloorSeparation) + radius * 1.3;
          const offsetZ = -random(options.length);

          aOffset.push(leftX, offsetY, offsetZ);
          aOffset.push(rightX, offsetY, offsetZ);
